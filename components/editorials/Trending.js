...
const Card = ({ img, name, dep, date, btnclr }) => {
    return (
        <div className='w-full min-h-[110px] md:min-h-[7rem] border-b-[1px] border-[#707070] border-opacity-40 flex'>
            <img src={img} className='h-[50px] md:h-[90px]' alt="" />
            <div className='flex flex-col justify-around ml-5 h-[70px] md:h-[100px]'>
                <h2 className='font-semibold text-[black] text-base md:text-lg font-sans'>{name}</h2>
                <p className='text-[#606060] text-xs md:text-sm font-proxima'>{dep}</p>
                <div className='flex justify-between w-[130%] items-center'>
                    ...
                    <p className='text-[#606060] text-xs font-proxima'>{date}</p>
                </div>
            </div>
        </div>
    )
...
const Trending = () => {
    return (
        <div className={`${classes.trendingnow} h-[483px] p-4 w-full md:w-[32%] mt-44 md:mt-0`}>
